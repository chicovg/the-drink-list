...
(def app (uix/create-context {}))

(defn- use-main-app-state
  []
  (let [[loading? set-loading!] (uix/use-state true)
        [page set-page!]        (uix/use-state :login)
        [user set-user!]        (uix/use-state nil)]
    {:loading?     loading?
...
(defn- use-main-page-state
  []
  (let [[search-term set-search-term!] (uix/use-state nil)
        [sort-state set-sort-state!]   (uix/use-state {:field :created
                                                       :asc?  false})]
...
(defn- use-delete-modal-state
  []
  (let [[delete-modal-drink-id set-delete-modal-drink-id!] (uix/use-state nil)
        [show-delete-modal? set-show-delete-modal!]        (uix/use-state false)
...
(defn- use-drink-modal-state
  []
  (let [[drink-modal-drink set-drink-modal-drink!] (uix/use-state nil)
        [show-drink-modal? set-show-drink-modal!]  (uix/use-state false)
...
(defn- use-drinks-state
  []
  (let [[drinks-map set-drinks-map!] (uix/use-state nil)
        drinks                       (->> drinks-map
                                          vals
...
(defn- use-favorites-panel-state
  [drinks]
  (let [[favorites-panel set-favorites-panel!]           (uix/use-state :style)
        [favorites-sort-state set-favorites-sort-state!] (uix/use-state {:field :average
                                                                         :asc?  false})
